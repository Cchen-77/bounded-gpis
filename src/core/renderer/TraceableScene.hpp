...
public:
    ...
    Vec3f hanldeInfinities(Ray& ray, IntersectionTemporary& data, IntersectionInfo& info, bool enableLightSampling, bool wasSpecular) const
    {
        ...
        for (const std::shared_ptr<Primitive>& p : _infiniteLights) {
            ...
            if (data.primitive) {
                ...
                if (!enableLightSampling || wasSpecular || !info.primitive->isSamplable()) {
                    emission += info.primitive->evalDirect(data, info);
                }
            }
        }
        return emission;
