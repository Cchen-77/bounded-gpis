...
#include"phasefunctions/GPISNEEPhaseFunction.hpp"
...
namespace Tungsten {
        ...
        MediumState & state, MediumSample & sample) const
    {
        ...
        if (auto NEECtxt = std::dynamic_pointer_cast<GPContextPratical1D>(state.gpContext)) {
            if (NEECtxt->enableNEE) {
                GPISNEEInfos NEEInfo;

                NEEInfo.rd = ray.dir();
                NEEInfo.gradZ = NEECtxt->gradZ;
                NEEInfo.kDerivative2 = NEECtxt->kDerivative2;
                NEEInfo.M = NEECtxt->M;
                NEEInfo.gradientMean = NEECtxt->gradientMean;

                state.NEEPhase = std::make_shared<GPISNEEPhaseFunction>(_phaseFunctions[state.lastGPId], NEEInfo);
                sample.phase = state.NEEPhase.get();
            }
            else {
                sample.phase = _phaseFunctions[state.lastGPId].get();
            }
