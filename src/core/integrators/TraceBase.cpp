...
Vec3f TraceBase::volumeSampleDirect(const Primitive &light,
                    ...
                    const Ray &parentRay)
{
    if (mediumSample.phase->isPureSpecular()||mediumSample.phase->isForward()) {
        return Vec3f(0.0f);
    }
...
{
    wasSpecular = mediumSample.phase->isPureSpecular() || !enableLightSampling;
        ...
        Vec3f throughput, bool wasSpecular, Vec3f &emission)
{
    ...
    emission += throughput * _scene->hanldeInfinities(ray, data, info, enableLightSampling, wasSpecular);
