...
namespace Tungsten {
	...
	extern std::array<uint8_t, 256> reverse8_lut;
	extern std::array<uint16_t, 65536> reverse16_lut;
	...
	template<>
	inline uint8_t reverse<uint8_t>(uint8_t x) {
		x = ((x & 0xF0) >> 4) | ((x & 0x0F) << 4);
		x = ((x & 0xCC) >> 2) | ((x & 0x33) << 2);
		x = ((x & 0xAA) >> 1) | ((x & 0x55) << 1);
		return x;
	...
	template<>
	inline uint16_t reverse<uint16_t>(uint16_t x) {
		x = (x >> 8) | (x << 8);
	...
	template<>
	inline uint32_t reverse<uint32_t>(uint32_t x) {
		return (uint32_t(reverse16_lut[x & 0xFFFF]) << 16) |
			(uint32_t(reverse16_lut[(x >> 16) & 0xFFFF]));
	}

	template<>
	inline uint64_t reverse<uint64_t>(uint64_t x) {
		return (uint64_t(reverse16_lut[x & 0xFFFF]) << 48) |
			(uint64_t(reverse16_lut[(x >> 16) & 0xFFFF]) << 32) |
			(uint64_t(reverse16_lut[(x >> 32) & 0xFFFF]) << 16) |
			(uint64_t(reverse16_lut[(x >> 48) & 0xFFFF]));
	}
	...
	struct KernelParams {
		double ls;
		static KernelParams Min() {
			return KernelParams{ -1e9 };
		}
		static KernelParams Max() {
			return KernelParams{ 1e9 };
		}
		inline void updateMax(const KernelParams& child) {
			ls = std::max(ls, child.ls);
		}
		inline void updateMin(const KernelParams& child) {
			ls = std::min(ls, child.ls);
		}
	};
	class ConvolutionFunctionLUT {
	public:
		//For simplify implementation, we only use block size of 8 or 16
		
		//typedef uint8_t BLOCK_TYPE;
		typedef uint16_t BLOCK_TYPE;
		static constexpr uint16_t BLOCK_BITS = 16;
		static constexpr uint16_t BLOCK_BIT_LOG2 = 4;
		
	protected:

		std::shared_ptr<const ConvolutionFunction> _h;
		double _marchingStepSize;
	public:
		ConvolutionFunctionLUT(std::shared_ptr<const ConvolutionFunction> h) : _h(h) {}
		virtual void initBundleLUT(double stepSize, int maxBundleSize) {
			_marchingStepSize = stepSize;
		}
		virtual double queryPerPoint(const Ray& ray, double t, int level, const BLOCK_TYPE* lutBlocks, int blockCount, const KernelParams& params) const = 0;
		virtual double queryPerPoint2(const Ray& ray, double t, int level, const BLOCK_TYPE* lutBlocks, int blockCount, const KernelParams& params) const = 0;
	};

	class ConvolutionFunction : public std::enable_shared_from_this<ConvolutionFunction> {
		friend class SEConvolutionLUT;
	public:
		double operator()(Vec3d pa, Vec3d pb) const {
			return h(pa, pb);
		}

		double operator()(Vec3d pa, Vec3d pb, const KernelParams& params) const {
			return h(pa, pb, params);
		}

		double operator()(const Ray& ray, double ta, double tb) const {
		
			return h_1d(ray, ta, tb);
		}

		double operator()(const Ray& ray, double ta, double tb, const KernelParams& params) const {
			return h_1d(ray, ta, tb, params);
		}


		double evalGradient(const Ray& ray, double ta, double tb) const {
			return dh_da_1d(ray, ta, tb);
		}

		double evalGradient(const Ray& ray, double ta, double tb, const KernelParams& params) const {
			return dh_da_1d(ray, ta, tb, params);
		}


		Vec3d evalGradient(Vec3d pa, Vec3d pb) const {
			return dh_da(pa, pb);
		}

		Vec3d evalGradient(Vec3d pa, Vec3d pb, const KernelParams& params) const {
			return dh_da(pa, pb, params);
		}
		...
		virtual double getMinRadius() const { return 0.; }
		virtual double getMaxRadius() const { return 0.; }
		virtual double getRadius(Vec3d p) const { return 0.; }

		virtual double getRadius(Vec3d p, const KernelParams& params) const {
			return getRadius(p, params);
		}


		virtual double getRadius(const Ray& ray, double t) const {
			auto ro = vec_conv<Vec3d>(ray.pos());
			auto rd = vec_conv<Vec3d>(ray.dir());
			return getRadius(ro + rd * t);
		}
		
		virtual double getRadius(const Ray& ray, double t, const KernelParams& params) {
			return getRadius(ray, t);
		}


		virtual KernelParams getParams(Vec3d p) const {
			return {};
		}

		virtual KernelParams getParams(const Ray& ray, double t) const {
			return {};
		}

		

		virtual bool sameLevel(const Ray& ray, const KernelParams& minParams, const KernelParams& maxParams, int& level) const {
			level = 0;
			return true;
		}

		virtual double getSigma(Vec3d p) const { return 0.; }

		virtual std::shared_ptr<ConvolutionFunctionLUT> makeLUT(int impulsePerCell) const { return nullptr; }

	public:
		virtual double h(Vec3d pa, Vec3d pb) const = 0;
		virtual double h(Vec3d pa, Vec3d pb, const KernelParams& params) const = 0;
		virtual Vec3d dh_da(Vec3d pa, Vec3d pb) const = 0;
		virtual Vec3d dh_da(Vec3d pa, Vec3d pb, const KernelParams& params) const = 0;
		virtual double h_1d(const Ray& ray, double ta, double tb) const = 0;
		virtual double h_1d(const Ray& ray, double ta, double tb, const KernelParams& params) const = 0;
		virtual double dh_da_1d(const Ray& ray, double ta, double tb) const = 0;
		virtual double dh_da_1d(const Ray& ray, double ta, double tb, const KernelParams& params) const = 0;

		virtual double norm1d(const Ray& ray, double ta, double tb, const KernelParams& kernelParams) const = 0;
	};

	// squared-exponential
	class SEConvolutionLUT : public ConvolutionFunctionLUT {
		int _impulsePerCell;
		double _lengthScale;
		double _radius;

		std::vector<std::array<double, 1 << BLOCK_BITS>> _LUT;
		std::vector<std::array<double, 1 << BLOCK_BITS>> _LUT2;
	public:
		SEConvolutionLUT(std::shared_ptr<const ConvolutionFunction> h, double impulsePerCell, double l, double radius);
		virtual double queryPerPoint(const Ray& ray, double t, int level, const BLOCK_TYPE* lutBlocks, int blockCount, const KernelParams& params) const;
		//for lb
		virtual double queryPerPoint2(const Ray& ray, double t, int level, const BLOCK_TYPE* lutBlocks, int blockCount, const KernelParams& params) const;

	};

	// squared-exponential varying length scale
	class SEVLConvolutionLUT : public ConvolutionFunctionLUT {


		static constexpr size_t ONESIDE_PARAM_RES = 8;
		...
		std::vector <std::array<std::array<std::vector<std::array<double, 1 << BLOCK_BITS>>, 2>, ONESIDE_PARAM_RES>> _LUT;

		std::vector<std::array<std::array<double, 1 << BLOCK_BITS>,2>> _LUT_lipchitz_local;
		
		std::vector<std::array<std::vector<std::array<double, 1 << BLOCK_BITS>>, 2>>  _LUT_lipchitz_left;
		std::vector<std::array<std::vector<std::array<double, 1 << BLOCK_BITS>>, 2>> _LUT_lipchitz_right;
	public:
		SEVLConvolutionLUT(std::shared_ptr<const ConvolutionFunction> h, double impulsePerCell, double minRadius, double maxRadius, Vec3f aniso);
		virtual double queryPerPoint(const Ray& ray, double t, int level, const BLOCK_TYPE* lutBlocks, int blockCount, const KernelParams& params) const;
		virtual double queryPerPoint2(const Ray& ray, double t, int level, const BLOCK_TYPE* lutBlocks, int blockCount, const KernelParams& params) const;
	};

	class SquaredExponentialConvolution : public ConvolutionFunction {
		float _l;
		float _sigma;
		Vec3f _aniso;
		...
		double _norm3D;
	public:
		SquaredExponentialConvolution(float l = 1.f, float sigma = 1.f, Vec3f aniso = Vec3f(1.f)) : _l(l), _sigma(sigma), _aniso(aniso) {
			_radius = 2 * _l / _aniso.min();
			_norm3D = std::pow(2. / (PI * sqr(_l)), 0.75) * std::pow(_aniso.x() * _aniso.y() * _aniso.z(), 0.25);
		};
		...
		virtual double getMinRadius() const {
			return 3 * _l / _aniso.max() / std::sqrt(2);;
		}
		virtual double getMaxRadius() const {
			return 3 * _l / _aniso.min() / std::sqrt(2);;
		}
		virtual double getRadius(Vec3d p) const {
			return 3 * _l / _aniso.min() / std::sqrt(2);;
		}
		...
		virtual double getRadius(const Ray& ray, double t) const {
			auto rd = vec_conv<Vec3d>(ray.dir());
			double effectiveLengthScale = _l / std::sqrt(rd.dot(Vec3d{ _aniso.x(), _aniso.y(), _aniso.z() }.cwiseProduct(rd)));

			return 3 * effectiveLengthScale / std::sqrt(2);;
		}

		virtual bool sameLevel(const Ray& ray, const KernelParams& minParams, const KernelParams& maxParams, int& level) const {

			auto rd = vec_conv<Vec3d>(ray.dir());
			double minRaidus = getMinRadius();
			
			double minR = 3 * minParams.ls / std::sqrt(rd.dot(Vec3d{ _aniso.x(), _aniso.y(), _aniso.z() }.cwiseProduct(rd)));
			double maxR = 3 * maxParams.ls / std::sqrt(rd.dot(Vec3d{ _aniso.x(), _aniso.y(), _aniso.z() }.cwiseProduct(rd)));
			int minLevel = std::max(0.,std::floor(std::log2(minR / minRaidus)));
			int maxLevel = std::max(0., std::floor(std::log2(maxR / minRaidus)));
			level = minLevel;

			return minLevel == maxLevel;
		}

		virtual double getSigma(Vec3d p) const { return _sigma; }

		virtual std::shared_ptr<ConvolutionFunctionLUT> makeLUT(int impulsePerCell) const {
			if (!isAnisotropic()) {
				return std::make_shared<SEConvolutionLUT>(shared_from_this(), impulsePerCell, _l, getMaxRadius());
			}
			return std::make_shared<SEVLConvolutionLUT>(shared_from_this(), impulsePerCell, getMinRadius(), getMaxRadius(), _aniso);
		}

	public:
		virtual double h(Vec3d pa, Vec3d pb) const {
			double q = dist2(pa, pb, _aniso);
			return _norm3D * exp(-(q / sqr(_l)));
		}

		virtual double h(Vec3d pa, Vec3d pb, const KernelParams& params) const {
			return h(pa, pb);
		}


		virtual Vec3d dh_da(Vec3d pa, Vec3d pb) const {
			Vec3d d = pb - pa;
			return 2. * h(pa, pb) * d.cwiseProduct(Vec3d(_aniso.x(), _aniso.y(), _aniso.z())) / sqr(_l);
		}

		virtual Vec3d dh_da(Vec3d pa, Vec3d pb, const KernelParams& params) const {
			return dh_da(pa, pb);
		}


		virtual double h_1d(const Ray& ray, double ta, double tb) const {
			double ansioRd = dist2(Vec3d(0., 0., 0.), vec_conv<Vec3d>(ray.dir()), _aniso);
			double norm1d = std::pow(2. * ansioRd / (PI * sqr(_l)), 0.25);
			double dt = tb - ta;

			return norm1d * exp(-ansioRd*sqr(dt) / sqr(_l));
		}

		virtual double h_1d(const Ray& ray, double ta, double tb, const KernelParams& params) const {
			return h_1d(ray, ta, tb);
		}

		virtual double dh_da_1d(const Ray& ray, double ta, double tb) const {
			double ansioRd = dist2(Vec3d(0., 0., 0.), vec_conv<Vec3d>(ray.dir()), _aniso);
			double dt = tb - ta;
			return 2 * h_1d(ray, ta, tb) * dt * ansioRd / sqr(_l);
		}

		virtual double dh_da_1d(const Ray& ray, double ta, double tb, const KernelParams& params) const {
			double ansioRd = dist2(Vec3d(0., 0., 0.), vec_conv<Vec3d>(ray.dir()), _aniso);
			double dt = tb - ta;
			return 2 * h_1d(ray, ta, tb) * dt * ansioRd / sqr(_l);
		}

		virtual double norm1d(const Ray& ray, double ta, double tb, const KernelParams& kernelParams) const {
			return std::pow(2. * dist2(Vec3d(0., 0., 0.), vec_conv<Vec3d>(ray.dir()), _aniso) / (PI * sqr(_l)), 0.25);
		};
	...
	public:
		virtual double eval_Isotropic(Vec3d p, double r) const {
			return 0.;
		}
		
		virtual double evalDerivative1_Isotropic(Vec3d p, double r) const {
			return 0.;
		}
		
		virtual double evalDerivative2_Isotropic(Vec3d p, double r) const {
			return 0.;
		}

		virtual Vec3d getAniso() const {
			return vec_conv<Vec3d>(_aniso);
		}
		...
		virtual double getMinSigma() const {
			return 1.;
		}
		...
		virtual std::shared_ptr<ConvolutionFunction> getConvolutionFunction() const { return nullptr; }


		virtual double getMaxCov(Vec3d c, const Ray& r, double t0, double t1, const KernelParams& minParams, const KernelParams& maxParams) { return 0.; }
		virtual double getMinCov(Vec3d c, const Ray& r, double t0, double t1, const KernelParams& minParams, const KernelParams& maxParams) { return 0.; }

		virtual double getMaxdCov(Vec3d c, Vec3d dir, const Ray& r, double t0, double t1, const KernelParams& minParams, const KernelParams& maxParams) { return 0.; }
		virtual double getMindCov(Vec3d c, Vec3d dir, const Ray& r, double t0, double t1, const KernelParams& minParams, const KernelParams& maxParams) { return 0.; }
	...
	public:
		...
		virtual double maxVal() const { return 0.; }
		virtual double minVal() const { return 0.; }
	};
	...
	class ProceduralNoise : public ProceduralScalar {
		...
		static std::string noiseTypeToString(NoiseType v) {
			...
			case NoiseType::LeftCenterRight: return "left_center_right";
		...
		static NoiseType stringToNoiseType(std::string v) {
			...
			else if (v == "left_center_right")
				return NoiseType::LeftCenterRight;
	...
	public:
		...
		virtual double operator()(Vec3d p) const override;
		virtual double maxVal() const override;
		virtual double minVal() const override;
	};
	...
	class SVLengthScaleSquaredExponentialConvolution : public ConvolutionFunction {
		std::shared_ptr<ProceduralScalar> _ls;
		float _minL;
		float _maxL;
		std::shared_ptr<ProceduralScalar> _sigma;
		Vec3f _aniso;

	public:
		SVLengthScaleSquaredExponentialConvolution(std::shared_ptr<ProceduralScalar> sigma = nullptr, std::shared_ptr<ProceduralScalar> ls = nullptr, float minL = 0.f, float maxL = 1.f, Vec3f aniso = Vec3f(1.f)) : _ls(ls), _minL(minL), _maxL(maxL), _sigma(sigma), _aniso(aniso) {
		...
		virtual double getMinRadius() const {
			return 3 * _minL / std::sqrt(_aniso.max()) / std::sqrt(2);
		}
		virtual double getMaxRadius() const {
			return 3 * _maxL / std::sqrt(_aniso.min()) / std::sqrt(2);
		}
		virtual double getRadius(Vec3d p) const {
			double lengthScale = (*_ls)(p);
			return 3 * lengthScale / std::sqrt(_aniso.min()) / std::sqrt(2);
		}
		...
		virtual double getRadius(const Ray& ray, double t) const {
			...
			double lengthScale = (*_ls)(ro + rd * t);
		...
		virtual double getSigma(Vec3d p) const {
			return (*_sigma)(p);
		}
	...
	public:
		...
		virtual double h(Vec3d pa, Vec3d pb) const {
			double lengthScale = (*_ls)(pa);
			double q = dist2(pa, pb, _aniso);
			double _norm3D = std::pow(2. / (PI * sqr(lengthScale)), 0.75) * std::pow(_aniso.x() * _aniso.y() * _aniso.z(), 0.25);;
			return _norm3D * exp(-(q / sqr(lengthScale)));
		}
		...
		virtual Vec3d dh_da(Vec3d pa, Vec3d pb) const {
			...
			double q = dist2(pa, pb, _aniso);
			double _norm3D = std::pow(2. / (PI * sqr(lengthScale)), 0.75) * std::pow(_aniso.x() * _aniso.y() * _aniso.z(), 0.25);;
			double h_val = _norm3D * exp(-(q / sqr(lengthScale)));


			auto gradL = [&](const Vec3d& p) -> Vec3d {
				double eps = 1e-4;
				Vec3d g;
				...
				for (int i = 0; i < 3; ++i) {
					...
					double f_plus = (*_ls)(p_plus);
					double f_minus = (*_ls)(p_minus);
			...
			Vec3d grad0 = 2. * h_val * d.cwiseProduct(Vec3d(_aniso.x(), _aniso.y(), _aniso.z())) / sqr(lengthScale);
			double dh_dL = h_val * (-1.5 / lengthScale + 2. * q / std::pow(lengthScale, 3));

			Vec3d grad1 = dh_dL * gradL(pa);
			return  grad0 + grad1;
		}
		...
		virtual double h_1d(const Ray& ray, double ta, double tb) const {
			double lengthScale = (*_ls)(vec_conv<Vec3d>(ray.pos() + ray.dir() * tb));
			double ansioRd = dist2(Vec3d(0., 0., 0.), vec_conv<Vec3d>(ray.dir()), _aniso);
			double norm1d = std::pow(2. * ansioRd / (PI * sqr(lengthScale)), 0.25);;
			double dt = tb - ta;

			return norm1d * exp(-ansioRd * sqr(dt) / sqr(lengthScale));
		}
		...
		virtual double dh_da_1d(const Ray& ray, double ta, double tb) const {
			auto pos_at = [&](double t) { return vec_conv<Vec3d>(ray.pos() + ray.dir() * t); };

			auto gradL = [&](double t) -> double {
				double eps = 1e-4;
				double L_plus = (*_ls)(pos_at(t + eps));
				double L_minus = (*_ls)(pos_at(t - eps));
				return (L_plus - L_minus) / (2.0 * eps);
				};

			double dt = tb - ta;
			Vec3d dir3 = vec_conv<Vec3d>(ray.dir());
			double ansioRd = dist2(Vec3d(0., 0., 0.), dir3, _aniso);

			double lengthScale = (*_ls)(pos_at(ta));
			double norm1d = std::pow(2. * ansioRd / (PI * sqr(lengthScale)), 0.25);

			double h_val = norm1d * exp(-ansioRd * sqr(dt) / sqr(lengthScale));


			double grad0 = 2. * h_val * dt * ansioRd / sqr(lengthScale);

			double dh_dL = h_val * (-0.25 / lengthScale + 2. * ansioRd * sqr(dt) / std::pow(lengthScale, 3));
			double grad1 = dh_dL * gradL(ta);

			return grad0;
		}

		virtual double dh_da_1d(const Ray& ray, double ta, double tb, const KernelParams& kernelParams) const {
			auto pos_at = [&](double t) { return vec_conv<Vec3d>(ray.pos() + ray.dir() * t); };
			...
			double lengthScale = kernelParams.ls;
			double norm1d = std::pow(2. * ansioRd / (PI * sqr(lengthScale)), 0.25);
		...
		virtual double norm1d(const Ray& ray, double ta, double tb, const KernelParams& kernelParams) const {
			double lengthScale = kernelParams.ls;
			return std::pow(2. * dist2(Vec3d(0., 0., 0.), vec_conv<Vec3d>(ray.dir()), _aniso) / (PI * sqr(lengthScale)), 0.25);
		};

	};

	class SVLengthScaleSquaredExponentialCovariance : public CovarianceFunction {
		Vec3f _aniso;
		std::shared_ptr<ProceduralScalar>  _sigma = nullptr;
		std::shared_ptr<ProceduralScalar>  _ls = nullptr;
		float _minL;
		float _maxL;
		float _maxSigma;
		float _minSigma;
	public:
		SVLengthScaleSquaredExponentialCovariance(std::shared_ptr<ProceduralScalar> sigma = nullptr, std::shared_ptr<ProceduralScalar> ls = nullptr,
			Vec3f aniso = Vec3f(1.f)) : _sigma(sigma), _ls(ls)
		{
			_aniso = aniso;
		}
		...
		virtual double eval_Isotropic(Vec3d p, double r) const {
			auto lengthScale = (*_ls)(p);
			auto sigma = (*_sigma)(p);
			return sqr(sigma) * exp(-(sqr(r) / (2 * sqr(lengthScale))));
		}

		virtual double evalDerivative1_Isotropic(Vec3d p, double r) const {
			auto lengthScale = (*_ls)(p);
			auto sigma = (*_sigma)(p);
			double l2 = sqr(lengthScale);
			return -sqr(sigma) * r / l2 * exp(-sqr(r) / (2 * l2));
		}
		...
		virtual double evalDerivative2_Isotropic(Vec3d p, double r) const {
			...
			double exp_term = exp(-sqr(r) / (2 * l2));
			return sigma2 * exp_term * (sqr(r) / (l2 * l2) - 1.0 / l2);
		}


		virtual std::string id() const {
			return tinyformat::format("svlse/aniso=[%.4f,%.4f,%.4f]-s=%.3f", _aniso.x(), _aniso.y(), _aniso.z(), _sigma);
		}

		virtual std::shared_ptr<ConvolutionFunction> getConvolutionFunction() const {
			return std::make_shared<SVLengthScaleSquaredExponentialConvolution>(_sigma, _ls, _minL, _maxL, _aniso);
		}
	...
	public:
		...
		virtual double eval_Isotropic(Vec3d p, double r) const {
			return sqr(_sigma) * exp(-(sqr(r) / (2 * sqr(_l))));
		}

		virtual double evalDerivative1_Isotropic(Vec3d p, double r) const {
			double l2 = sqr(_l);
			return -sqr(_sigma) * r / l2 * exp(-sqr(r) / (2 * l2));
		}

		virtual double evalDerivative2_Isotropic(Vec3d p, double r) const {
			double l2 = sqr(_l);
			double sigma2 = sqr(_sigma);
			double exp_term = exp(-sqr(r) / (2 * l2));
			return sigma2 * exp_term * (sqr(r) / (l2 * l2) - 1./l2);
		}
		...
		virtual std::shared_ptr<ConvolutionFunction> getConvolutionFunction() const {
			return std::make_shared<SquaredExponentialConvolution>(_l, _sigma, _aniso);
		}
